    ...
    async function process(cur_country_code) {
        do {
            ...
            await executeWithRetry("页面num选择", async function () {
                let tempBtn = modalDiv.querySelector('div[title="50/页"]') || modalDiv.querySelector('div[title="50 / Page"]');
                if (tempBtn) {
                    tempBtn.click();
                    return true; // 假设条件始终为真
                }
            ...
            await executeWithRetry("1000/页", async function () {
                let tempBtn = Array.from(popupdivElement.querySelectorAll('span')).find(span => span.textContent.trim() === '1000/页' || span.textContent.trim() === '1000 / Page');
                if (tempBtn) {
                    tempBtn.click();
                    return true; // 假设条件始终为真
                }
            ...
            if (firstExecute) {
                ...
                console.log("开始执行选择页码");
                // // 点击秒杀
                await executeWithRetry("开始执行选择页码"+curCount, async function () {
                ...
                await sleep(2000);
                await executeWithRetry("开始等待1000商品加载完成",async function () {
                    var tempDiv = document.querySelector('.theme-arco-pagination');
                    console.log(tempDiv.style.visibility);
                    if (tempDiv.style.visibility == "visible") {
                        return true;
                    }
                    return false;
            ...
            await sleep(1000);
            const sucessModal = document.querySelector(".theme-arco-modal-footer")
            if (sucessModal) {
                sucessModal.querySelector("button").click();
            }
            await sleep(500);
            repeatCount--;
            curCount++;
